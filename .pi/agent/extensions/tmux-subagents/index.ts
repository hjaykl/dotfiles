...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("tool-calls", {
    ...
    handler: async (_args, ctx) => {
      ...
      try {
        // Use the session ID ("$N"), not the name: a numeric session name like
        // "0" is parsed by tmux as a window index when used as a target.
        const session = execSync("tmux display-message -p '#{session_id}'", {
          encoding: "utf-8",
        }).trim();
