...
function currentSession(): string {
  // Return the session ID (e.g. "$0"), NOT the session name. tmux target
  // parsing treats a bare numeric value as a WINDOW INDEX, so a session named
  // "0" passed as `-t 0` is read as window index 0 — which collides ("index 0
  // in use") the moment a second window is created, breaking parallel spawns.
  // Session IDs are always "$N" and unambiguous as targets.
